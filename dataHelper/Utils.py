...
def load_audio_only(audio_datafile):
    audio_data = read_audio(audio_datafile)
    npaudios = np.array(audio_data)
    return npaudios


def load_image_only(image_data_dir):
    videos = []
    label_dirs = sample(os.listdir(image_data_dir), 20)
    label_dirs.sort()
    for img_name in label_dirs:
